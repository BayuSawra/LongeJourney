...
1. 新增或修改 lore 条目、编写全新剧情内容前，先读取 `lore/INDEX.md` 及相关条目；已定稿的 timeline 迁移按 `docs/task_checkpoint.md` 的“下一步”先执行，本回合实际修改 lore 条目时再做增量核对，不在动手前做全量重读。
...
校验由变更驱动：只有本回合实际修改了 lore 文件才触发，且只做增量核对；它不是提交前的例行全量动作，每个提交周期最多执行一轮。

1. 本回合未修改 `lore/` 下任何文件（含 `plot-threads.md`）时，不校验、不登记、不重读，直接继续或结束。
2. 仅当本回合实际新增或修改了 lore 条目时，对本次变更影响的增量范围核对：名字与拼写、年龄与状态、关系、所在地、当前时间、已知信息、物品归属；不跑全量检查。
3. `tools/check_lore_canon.py` 只在明确需要全量核验时运行（索引大改、批量迁移、用户明确要求），不作为例行提交动作。
4. `tools/update_lore_index.py` 只在新增/删除条目、条目摘要变化或 `plot-threads.md` 变化时运行；其余情况跳过。
5. 校验失败时，只修复脚本报出的问题并重新运行一次；仍未通过则停止并报告，不进入修复-校验循环。
6. 登记只写最终一行；已登记且条目未变更、阶段未切换时直接沿用，不重复登记、不追加。格式：`[已检查] 范围=<范围> 结果=<结果> 登记=<YYYY-MM-DD HH:mm> 线程=<线程标识>`。
...
- 检查登记：登记只发生在收尾时，不当作每回合动作。`docs/task_checkpoint.md` 已登记且条目未变更、阶段未切换时直接沿用，不重读、不重推演、不重复登记；本回合无 lore 变更时不校验、不登记。确需登记时只写最终一行，格式为 `[已检查] 范围=<范围> 结果=<结果> 登记=<YYYY-MM-DD HH:mm> 线程=<线程标识>`。
