...
Windows、Python 3.10+、PowerShell，验证固定使用 Godot **4.6.2 standard**。
保留项目 4.6 / GL Compatibility 配置，不要求 .NET SDK。资源导入固定串行，避免本机冷导入字体时观察到的并发崩溃。
...
```powershell
$env:GODOT_PATH = "$PWD/.local-tools/godot-4.6.2/Godot_v4.6.2-stable_win64_console.exe"
