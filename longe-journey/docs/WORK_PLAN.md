...
- 10.1 timeline/lore/资源导入导出脚本：输入输出格式与文件路径明确。
  ...
  - 10.1.4 资源清单与引用校验导入导出：清单化 `art/`、`font/` 等资源，导入后校验引用完整。验证通过就执行，不需要来回反复验证，收窄你的工作流。后续如需测试，godot安装在E:\Godot\Godot_v4.6.2-stable_mono_win64
- 10.2 备份目录规范：备份位置、命名与保留规则明确。验证通过就执行，不需要来回反复验证，收窄你的工作流。
- 10.3 往返验证：导出内容可重新导入且校验通过。如需测试，godot安装在E:\Godot\Godot_v4.6.2-stable_mono_win64
- 10.4 文档：把脚本用法与备份规范写回本计划/工具文档。
- 建立 timeline、lore、资源的批量导入导出脚本与备份目录规范。
- 验证：导出的内容可重新导入且校验通过。验证通过就执行，不需要来回反复验证，收窄你的工作流。后续如需测试，godot安装在E:\Godot\Godot_v4.6.2-stable_mono_win64
