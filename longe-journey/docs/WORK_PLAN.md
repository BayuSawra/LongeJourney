...
- 9.1 从 `lore/INDEX.md`/canon 导入运行时数据，形成只读设定数据。
- 9.2 图鉴/设定浏览 UI：分类列出地点、事件、角色等。
- 9.3 搜索/筛选/详情：可按关键词或分类查找，详情展示原文。
- 9.4 校验流程：与 `tools/check_lore_canon.py` 的结果保持一致。
...
- 10.2 备份目录规范：备份位置、命名与保留规则明确。
- 10.3 往返验证：导出内容可重新导入且校验通过。
